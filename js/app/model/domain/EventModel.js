define(['marionette',
        ...
        function(Marionette, Incidents, Subscriptions) {
            ...
            return Marionette.Controller.extend({
                dependencies: 'socket=socketService, eventScheduleModel, eventCache, frontPageModel, eventDetailsModel, keyMarketsModel, commands, vent',
                incidentEventXref:[],
...
//                        var keyValues='';
//                        var keyTypes = this.keysMarketsModel.getMarkets();
//
//                        for (key in keyTypes) {
...
//                },
                ...
                onEventChange: function(data) {
                    if (_.has(data, 'id')) {
                        var event = this.eventCache.getEvent(data.id);
                        if (event) {
                ...
                parseEventTradingState: function(eventTradingState) {
                    var event = this.eventCache.getEvent(eventTradingState.id);
                    if (event) {
...
//                    incidentModel.consolidate(incidents);
                ...
                onEventDetailsUpdate: function(detailsModel) {
                    ...
                    var eventScheduleEvent = this.eventCache.getEvent(details.id);
                ...
                parseMatchDetails: function(matchDetails) {
                    if (_.has(matchDetails, 'participants')) {
                        ...
                        this.schedule().setParticipants(eventId,participantArray);
                    }
                },
                ...
                /**
                 * Returns the current schedule from the schedule model
                 * @returns {*}
                 */
                schedule: function() {
                    return this.frontPageModel.getSchedule();
                }
